...
async def test_process_link_command_no_token(mock_message, mock_bp_api):
    """Тест: пользователь отправил /link без кода."""
    # Arrange
    mock_message.get_args.return_value = ""

    # Act
    await process_link_command(mock_message, mock_bp_api)

    # Assert
    mock_message.answer.assert_called_once_with(BotMessages.LINK_NO_CODE)
    mock_bp_api.complete_link.assert_not_called()


async def test_process_link_command_success(mock_message, mock_bp_api):
    """Тест: пользователь отправил /link с верным кодом."""
    # Arrange
    token = "VALID-CODE"
    mock_message.get_args.return_value = token
...
async def test_process_link_command_api_error(mock_message, mock_bp_api):
...
async def test_process_link_command_api_exception(mock_message, mock_bp_api):
    ...
    # Assert
    mock_bp_api.complete_link.assert_called_once_with(mock_message.from_user.id, token)
    mock_message.answer.assert_called_once_with(BotMessages.LINK_ERROR)
