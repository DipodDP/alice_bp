...
class Command(BaseCommand):
    help = "Migrates plaintext telegram_user_id to HMAC-SHA256 hashed values."
    ...
    def handle(self, *args, **options):
        ...
        users_to_migrate = AliceUser.objects.filter(telegram_user_id__isnull=False)
        ...
        try:
            with transaction.atomic():
                ...
                for user in users_to_migrate:
                    ...
                    if user.telegram_user_id and re.match(r'^[a-f0-9]{64}$', user.telegram_user_id):
                        self.stdout.write(f"Skipping user {user.id} (already hashed).")
                        continue

                    original_id = user.telegram_user_id
                    ...
                    if dry_run:
                        ...
                        user.telegram_user_id = hashed_id
                        user.save()
                        self.stdout.write(
