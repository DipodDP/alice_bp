...
class TelegramIdMigrationTest(TestCase):
    def test_migration_command(self):
        user = AliceUser.objects.create(telegram_user_id='12345')
        out = StringIO()
        ...
        self.assertEqual(user.telegram_user_id, expected_hash)
    ...
    def test_migration_command_dry_run(self):
        ...
        user.refresh_from_db()
        self.assertEqual(user.telegram_user_id, '12345')  # Should not have changed
    ...
    def test_migration_command_already_hashed(self):
        ...
        user = AliceUser.objects.create(telegram_user_id=hashed_id)

        out = StringIO()
        call_command('migrate_telegram_ids', stdout=out)

        user.refresh_from_db()
        self.assertEqual(user.telegram_user_id, hashed_id)
        self.assertIn(f'Skipping user {user.id}', out.getvalue())

    @override_settings(TELEGRAM_ID_HMAC_KEY=None)
    def test_no_secret_key(self):
        AliceUser.objects.create(telegram_user_id='12345')
        out = StringIO()
        call_command('migrate_telegram_ids', stdout=out)
        self.assertIn(
