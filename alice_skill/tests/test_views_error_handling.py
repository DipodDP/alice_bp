...
class ViewErrorHandlingTest(APITestCase):
    ...
    def test_generate_link_token_view_exception_handling(self):
        ...
        with patch(
            'alice_skill.views.generate_link_token',
            side_effect=Exception('Test exception'),
        ) as mock_generate_link_token:
            response = self.client.post(
                self.generate_link_token_url,
    ...
    def test_unlink_view_exception_handling(self, mock_user_filter):
        ...
        self.assertEqual(
            response.data['message'], 'Test exception'
        )  # The view returns the exception message directly
