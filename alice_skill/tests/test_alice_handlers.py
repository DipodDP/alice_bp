...
class AliceHandlerAPITestCase(APITestCase):
    def setUp(self):
        ...
        self.hashed_telegram_user_id = get_hashed_telegram_id(self.telegram_user_id)
    ...
    def test_link_account_handler_success(self, mock_system_random):
        ...
        response = self.client.post(
            f'{self.url}?token={self.token}', data, format='json'
        )
    ...
    @patch('alice_skill.services.secrets.SystemRandom')
    def test_link_account_handler_success_with_split_number_tokens(
        self, mock_system_random
    ):
        """
        Ensure the webhook triggers the link account handler and successfully links accounts
    ...
    @patch('alice_skill.services.secrets.SystemRandom')
    def test_link_account_handler_success_without_trigger_phrase(
        self, mock_system_random
    ):
        """
        Ensure the webhook triggers the link account handler and successfully links accounts
